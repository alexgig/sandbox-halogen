...
handleAction action =
    case action of
        ...
        FetchUser ->
            do --TODO What tells us that we are operating on the Halogen Monad in this do block?
                username <- H.gets _.username
                H.modify_ (_ {loading = true})
                response <- H.liftAff $ AX.get AXRF.json ("https://api.github.com/users/" <> username)
                H.modify_ (_ {loading = false, response =  do
                    body <- J.toObject =<< _.body <$> (hush response)
                    name <- J.toString =<< Object.lookup "name" body
                    pure name
                })
